        ...
        let rows = stmt.query_map(
            ...
            |row| {
                Ok(ViewLayoutCardItem {
                    ...
                    text_direction: row.get(12)?,
                    font_weight: row.get(13)?,
                    text_align: row.get(14)?,
                    padding: row.get(15)?,
                    padding_top: row.get(16)?,
                    padding_right: row.get(17)?,
                    padding_bottom: row.get(18)?,
                    padding_left: row.get(19)?,
                    border_radius: row.get(20)?,
                    show_label: row.get::<_, Option<i64>>(21)?.map(|value| value != 0),
                    has_override: row.get::<_, bool>(22)?,
                })
            },
        )?;

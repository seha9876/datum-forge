...
Excelインポートもメタテーブルを増やさず、CSVインポートと同じユーザー定義テーブルへ取り込む。

- 対象は `.xlsx` / `.xlsm` 内でExcelの「テーブル」として定義された範囲のみとする。
- 取り込み前にExcelテーブル選択、列マッピング、先頭行プレビュー、差分件数確認を行う。
- インポート方式と既存レコード照合キーはCSVインポートと同じで、`id` を使う。
- 最後に使用したExcelテーブル名は `.local/settings.json` に保存し、DBのERには影響しない。
